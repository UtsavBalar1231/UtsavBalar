...
1. **Audio Codec Driver**:
	...
	- Exposed sysfs attributes for runtime codec parameter configuration
...
3. **UART-Based IPC Protocol**:
	- Developed 4-byte fixed-size packet format for data exchange between Linux host and RP2040 microcontroller
	- Implemented CRC8 checksum validation on all packets
	- Designed command set supporting input events, LED control, power management, diagnostics, and system management
...
4. **Serio-Based Input Driver**:
	...
	- Added interrupt-driven handling and 50ms debounce timer for button events
</details>
...
Designed and implemented a Qt-based application for **PamirAI**, a startup specializing in AI-driven solutions. The application integrates with MCP-based AI modules via gRPC and renders at 60fps on 1080p displays for monitoring and controlling Edge AI hardware.
